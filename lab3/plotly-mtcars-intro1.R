gg#### Brief introduction to R Plotly package ####
...
p= plot_ly(data=airquality, x=~Wind, y=~Solar.R, mode="markers",
           color = ~Ozone)
p

str(mtcars)
p= plot_ly(data=airquality, x=~Wind, y=~Temp, mode="markers",
           color = ~as.factor(Month))
p
